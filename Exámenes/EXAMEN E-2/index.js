...
function multiplos() {  //Esta función es un pelín complicada
	...
	if (document.getElementById("number").value == "") {
		...
		for (multiplier = 1 * number / Math.abs(number); Math.abs(multiplier) <= Math.abs(number); multiplier += number / Math.abs(number)) {  //Recorremos todos los números entre el 1 y el número introducido (el rollo de number/Math.abs(number) lo utilizo para poder tener tablas con números negativos, aunque puede que exista algúna forma mejor)
...
function compIdentidad() {
	...
	if (password == "" || username == "") { //Si el password o el username no contienen nada, cancelamos
		alert("Cancelado");
